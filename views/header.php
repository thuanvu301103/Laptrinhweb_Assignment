<header class="flex-row h-25 w-100 gap-5 justify-content-between bg-white d-flex align-items-center p-2">
  ...
  <div class="flex-row gap-2 align-items-center w-25">
    <!-- search bar -->
    <form class="m-0 search-box position-relative">
      <label class="d-flex flex-row gap-1 bg-body-secondary rounded-pill align-items-center px-2">
        ...
        <!-- results -->
        <div class="result flex-column gap-3 position-absolute w-100 bg-light rounded-4 p-4" style="top: 150%; left: 0; z-index: 999; display: none;"></div>
      </label>
    </form>
  </div>
  <div class="flex-row gap-2 align-items-center flex-nowrap d-flex">
    ...
    <a href="<?php if (isset($_SESSION['id'])) {
                echo "./cart?userid=" . $_SESSION['id'];
              } ?>" title="Cart" class="bg-white btn-light btn fw-medium d-flex justify-content-center align-items-center">
